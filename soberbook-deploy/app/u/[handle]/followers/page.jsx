...
export default async function Page({ params }) {
  ...
  const { data: rows } = await supabase.rpc('followers_of', { h: handle });
  const list = rows || [];
  ...
  return (
    <>
      ...
      <div className="bar">FOLLOWERS</div>
      <div className="pad">
        <p className="hint" style={{ marginTop: 0 }}>
          {who.display_name}&rsquo;s followers
        </p>
        {list.length === 0 ? (
          <div className="empty">
            <div className="h">Nobody yet.</div>
            <div className="p">That is not a measure of anything.</div>
          </div>
        ) : (
